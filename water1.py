...
def is_goal(s):
    # It is a goal when one bottle X,Y,Z contains 10 liters
    if (s.x == 10):
        return True
    elif (s.y == 10):
        return True
...
def successors(s):
    ...
    # Case 7: Try to pour water from X to Y
    ...
    # Case 8: Try to pour water from X to Z
    ...
    # Case 9: Try to pour water from Y to X
    ...
    # Case 10: Try to pour water from Y to Z
    t = 7-s.z   # available space of Z
    if s.y > 0 and t > 0:
        if s.y > t:
            # Pour until Z is full
    ...
    # Case 11: Try to pour water from Z to X
    t = 19-s.x   # available space of X
    if s.z > 0 and t > 0:
        if s.z > t:
            # Pour until X is full
